    ...
    fun mapCursorToArrayList(notesCursor: Cursor?): ArrayList<Quote> {
        ...
        notesCursor?.apply {
            while (moveToNext()) {
                ...
                val writer =
                    getString(getColumnIndexOrThrow(DatabaseContract.QuoteColumns.WRITER))
                val place =
                    getString(getColumnIndexOrThrow(DatabaseContract.QuoteColumns.PLACE))
                quotesList.add(Quote(id, title, description,category, date, writer, place))
            }
        }
        return quotesList
    }
